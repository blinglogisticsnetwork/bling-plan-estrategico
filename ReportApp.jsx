...
function ReportApp(){
  ...
  const[printMode,setPrintMode]=useState(null) // null | 'executive' | 'visual'
  ...
  const handleLogin=(t)=>{setToken(t);loadData(t).then(d=>{setData(d);setLoading(false)})}
  const logout=()=>{localStorage.removeItem('bling_token');setToken(null);setData(null)}

  const printAll=async(mode)=>{
    setPrintMode(mode)
    setTimeout(()=>{window.print();setTimeout(()=>setPrintMode(null),1000)},300)
  }
  ...
  return <div style={{background:DARK,minHeight:'100vh',display:'flex',flexDirection:'column',fontFamily:"'Segoe UI',sans-serif",color:TEXT}}>
    ...
    <div style={{background:CARD,borderBottom:`1px solid ${BORDER}`,padding:'10px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <a href="/" style={{color:MUTED,fontSize:11,fontFamily:'monospace',textDecoration:'none'}}
