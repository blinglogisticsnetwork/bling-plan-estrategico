...
async function loadData(token,year){
  try{
    // Load both plan data (for the selected year) and BSC data
    const planKey=year===2026?'bling_v4':`bling_v4_${year}`
    const [r1,r2]=await Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/plan_data?key=eq.${planKey}`,{headers:{'apikey':SUPABASE_KEY,'Authorization':`Bearer ${token}`}}),
...
function MarcosView({data,print}){
  const TABS=[
  ...
  if(print){
    return <div>
      {TABS.map(t=><Block key={t.id} title={t.label} color={BLUE}>{t.content()}</Block>)}
    </div>
  }
  return <div>
...
function ReportApp(){
  ...
  const[fromYear,setFromYear]=useState(+(sessionStorage.getItem('rFrom')||new Date().getFullYear()))
  const[toYear,setToYear]=useState(+(sessionStorage.getItem('rTo')||new Date().getFullYear()))
  const[planYear]=useState(+(sessionStorage.getItem('rYear')||2026))
  ...
  useEffect(()=>{
    ...
    loadData(token,planYear).then(d=>{setData(d);setLoading(false)})
  },[])
  ...
  const printAll=(mode)=>{setExportError(null);setExporting(true);setPrintMode(mode)}

  // Once the print layout is mounted, render it straight to a downloadable PDF — no native print dialog.
  // Captured section-by-section (cover + one per module) instead of one giant canvas: a 16-page report
  // rendered as a single canvas can exceed the browser's max canvas size and fail silently.
  useEffect(()=>{
    if(!printMode)return
    let cancelled=false
    const run=async()=>{
      await new Promise(r=>setTimeout(r,300)) // let the print layout paint first
      if(cancelled)return
      const el=printRef.current
      if(!el){setPrintMode(null);setExporting(false);return}
      try{
        const sections=Array.from(el.children).filter(c=>c.tagName==='DIV')
        ...
        const bg=printMode==='executive'?'#ffffff':'#0A0F1E'
        let firstPage=true
        for(const section of sections){
          if(cancelled)return
          const canvas=await html2canvas(section,{scale:2,useCORS:true,backgroundColor:bg})
          const imgW=usableW
          ...
          while(sYmm<imgH){
            ...
            firstPage=false
            const sliceHmm=Math.min(imgH-sYmm,usableH)
            const sliceCanvas=document.createElement('canvas')
            sliceCanvas.width=canvas.width
            sliceCanvas.height=Math.round(sliceHmm*pxPerMm)
            const ctx=sliceCanvas.getContext('2d')
            ctx.drawImage(canvas,0,Math.round(sYmm*pxPerMm),canvas.width,sliceCanvas.height,0,0,canvas.width,sliceCanvas.height)
            pdf.addImage(sliceCanvas.toDataURL('image/jpeg',0.95),'JPEG',margin,margin,imgW,sliceHmm)
            sYmm+=sliceHmm
          }
        }
        if(cancelled)return
        const label=printMode==='executive'?'Ejecutivo':'Visual'
        pdf.save(`Bling-Plan-Estrategico-${planYear}-${label}.pdf`)
      }catch(err){
        console.error('PDF export failed',err)
        if(!cancelled)setExportError(err?.message||'Error desconocido al generar el PDF')
      }finally{
        if(!cancelled){setPrintMode(null);setExporting(false)}
      }
    }
    run()
  ...
  if(printMode){
    ...
    return <div ref={printRef} style={{background:bg,color:textC,fontFamily:"'Segoe UI',sans-serif",padding:'0',minHeight:'100vh'}}>
      <style>{`
      ...
      <div style={{textAlign:'center',padding:'60px 40px',background:isExec?'#f8f9fa':CARD,marginBottom:40,borderRadius:isExec?0:12}}>
        ...
        <h2 style={{fontSize:18,color:isExec?'#444':MUTED,fontWeight:'normal',marginBottom:4}}>Plan Estratégico {planYear}</h2>
      ...
      {MODULES.map((mod,idx)=>{
        ...
        return <div key={mod.id} className={idx>0?'page-break':''} style={{padding:'30px 40px',marginBottom:20}}>
          ...
          <View data={mod.id==='bsc'?{...data,__bsc_kpis:filterBscKpis(data.__bsc_kpis)}:data} print/>
        </div>
      })}
    </div>
  }
  ...
  return <div style={{background:DARK,minHeight:'100vh',display:'flex',flexDirection:'column',fontFamily:"'Segoe UI',sans-serif",color:TEXT}}>
    ...
    <div style={{background:CARD,borderBottom:`1px solid ${BORDER}`,padding:'10px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <img src={LOGO_WHITE} alt="Bling" style={{height:28,objectFit:'contain'}}/>
        <div>
          <div style={{fontSize:13,fontWeight:'bold'}}>Bling Logistics Network</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:'monospace',letterSpacing:1}}>REPORTES Y DOCUMENTOS — {planYear}</div>
        </div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <button onClick={()=>printAll('executive')} disabled={exporting} style={{background:'#ffffff',color:'#1a1a1a',border:'none',borderRadius:6,padding:'7px 14px',fontWeight:'bold',cursor:exporting?'default':'pointer',fontSize:12,fontFamily:'monospace',opacity:exporting?0.6:1}}>{exporting?'⏳ Generando...':'📄 Reporte Ejecutivo'}</button>
        <button onClick={()=>printAll('visual')} disabled={exporting} style={{background:A,color:DARK,border:'none',borderRadius:6,padding:'7px 14px',fontWeight:'bold',cursor:exporting?'default':'pointer',fontSize:12,fontFamily:'monospace',opacity:exporting?0.6:1}}>{exporting?'⏳ Generando...':'🎨 Presentación Visual'}</button>
    ...
    {exportError&&<div style={{background:`${RED}18`,borderBottom:`1px solid ${RED}55`,padding:'10px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexShrink:0}}>
      <span style={{fontSize:12,color:RED}}>⚠️ No se pudo generar el PDF: {exportError}</span>
      <button onClick={()=>setExportError(null)} style={{background:'transparent',border:`1px solid ${RED}`,color:RED,borderRadius:6,padding:'4px 10px',cursor:'pointer',fontSize:11}}>Cerrar</button>
    </div>}
    ...
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
      ...
      <div style={{width:220,background:'#0D1526',borderRight:`1px solid ${BORDER}`,overflowY:'auto',flexShrink:0,padding:'12px 0'}}>
        ...
        <div style={{borderTop:`1px solid ${BORDER}`,margin:'14px 0',padding:'10px 14px 0'}}>
          ...
          <button onClick={()=>printAll('executive')} disabled={exporting} style={{width:'100%',background:'#ffffff',color:'#1a1a1a',border:'none',borderRadius:6,padding:'8px',fontWeight:'bold',cursor:exporting?'default':'pointer',fontSize:11,marginBottom:6,opacity:exporting?0.6:1}}>{exporting?'⏳ Generando...':'📄 Reporte Ejecutivo'}</button>
          <button onClick={()=>printAll('visual')} disabled={exporting} style={{width:'100%',background:A,color:DARK,border:'none',borderRadius:6,padding:'8px',fontWeight:'bold',cursor:exporting?'default':'pointer',fontSize:11,opacity:exporting?0.6:1}}>{exporting?'⏳ Generando...':'🎨 Presentación Visual'}</button>
        </div>
      </div>
