...
const NAV=[
  {sec:"Análisis de Negocio",items:[
    {id:"dashboard",ic:"⊞",label:"Dashboard"},
    {id:"contexto",ic:"🏢",label:"Contexto"},
    {id:"marcos",ic:"🧭",label:"Marcos Estratégicos"},
    {id:"finanzas",ic:"📈",label:"Finanzas"},
    {id:"gtm",ic:"🚀",label:"Estrategia de Salida al Mercado"},
    {id:"competitivo",ic:"⚔️",label:"Análisis Competitivo"},
    {id:"pvs",ic:"💎",label:"Propuestas Únicas de Venta"},
    {id:"mvp",ic:"🗺",label:"Camino a un MVP"},
    {id:"cliente",ic:"👤",label:"Perfil del Cliente"},
    {id:"reportes",ic:"📄",label:"Reportes y PDF"},
  ]},
  {sec:"Medición Estratégica",items:[
    {id:"bsc",ic:"🎯",label:"Cuadro de Mando Integral"},
  ]},
  {sec:"Herramientas",items:[
    {id:"pitch",ic:"🎤",label:"Presentación de pitch",dis:true},
  ]},
];
...
function ReportesView(){
  ...
  const label=fromYear===toYear?`${fromYear}`:`${fromYear} — ${toYear}`;
  return <div style={{textAlign:"center",padding:"40px 20px"}}>
    ...
    <p style={{color:MUTED,fontSize:14,marginBottom:28,maxWidth:440,margin:"0 auto 24px"}}>Genera tu plan estratégico completo en PDF. Elige el período y el estilo.</p>
    {/* Selector período */}
    <div style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:12,padding:20,marginBottom:24,maxWidth:420,margin:"0 auto 24px",display:"flex",alignItems:"center",justifyContent:"center",gap:12,flexWrap:"wrap"}}>
      <span style={{fontSize:12,color:MUTED,fontFamily:"monospace"}}>📅 PERÍODO:</span>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:12,color:MUTED}}>Desde:</span>
        <select value={fromYear} onChange={e=>setFromYear(+e.target.value)}
          style={{background:"#0D1526",border:`1px solid ${A}`,borderRadius:6,color:A,padding:"6px 10px",fontSize:13,outline:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:"bold"}}>
          {years.map(y=><option key={y} value={y}>{y}</option>)}
        </select>
        <span style={{fontSize:12,color:MUTED}}>Hasta:</span>
    ...
    <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
      <a href={`#report?from=${fromYear}&to=${toYear}`} onClick={e=>{e.preventDefault();window.location.hash=`#report`;sessionStorage.setItem('report_from',fromYear);sessionStorage.setItem('report_to',toYear);}} style={{textDecoration:"none"}}>
        <div style={{background:"#ffffff",border:"2px solid #ffffff",borderRadius:12,padding:"20px 28px",cursor:"pointer",minWidth:180}}
          onMouseEnter={e=>e.currentTarget.style.borderColor=A} onMouseLeave={e=>e.currentTarget.style.borderColor="#ffffff"}>
          <div style={{fontSize:28,marginBottom:8}}>📋</div>
          <div style={{fontSize:14,fontWeight:"bold",color:"#1a1a1a",marginBottom:4}}>📄 Reporte Ejecutivo</div>
          <div style={{fontSize:12,color:"#666"}}>Fondo blanco · Estilo formal</div>
          <div style={{fontSize:11,color:A,marginTop:6,fontWeight:"bold"}}>{label}</div>
        </div>
      </a>
      <a href="#report" onClick={e=>{e.preventDefault();window.location.hash=`#report`;sessionStorage.setItem('report_from',fromYear);sessionStorage.setItem('report_to',toYear);sessionStorage.setItem('report_mode','visual');}} style={{textDecoration:"none"}}>
        <div style={{background:`${A}22`,border:`2px solid ${A}`,borderRadius:12,padding:"20px 28px",cursor:"pointer",minWidth:180}}>
          <div style={{fontSize:28,marginBottom:8}}>🎨</div>
          <div style={{fontSize:14,fontWeight:"bold",color:A,marginBottom:4}}>🎨 Presentación Visual</div>
          <div style={{fontSize:12,color:MUTED}}>Fondo oscuro · Estilo app</div>
          <div style={{fontSize:11,color:A,marginTop:6,fontWeight:"bold"}}>{label}</div>
        </div>
      </a>
    </div>
